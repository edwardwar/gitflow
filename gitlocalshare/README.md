...
mkdir Test.git
```
2.进入该文件夹，初始化git仓库(可以重建裸仓库，只允许拉取和推送 git init --bare)
```java
git init
```
...
将根仓库共享。
1.右键根仓库目录—>属性—>共享—>高级共享—->勾选共享此文件夹—->权限如下,要对所有人开发:

<img src="images/git_02.png">

2.在另一台机器B,上右键计算机—>映射网络驱动器：
输入根仓库地址比如： \\IP\仓库名称
...
3.上述步骤将根仓库（机器A）的地址映射为了Z:（机器B），想要提交到该地址，需要将机器B的本地仓库关联到机器B的本地的Z:目录下。其他拉取和提交的步骤和在机器A中一样。
